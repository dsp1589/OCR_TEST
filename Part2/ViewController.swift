...
extension ViewController : UINavigationControllerDelegate,UIImagePickerControllerDelegate, ImageEditingCompletedDelegate{
    
    func imagePickerController(_ picker: UIImagePickerController, didFinishPickingMediaWithInfo info: [String : Any]) {
        print("")
        if let imageCaptured = info[UIImagePickerControllerEditedImage] as? UIImage{
            picker.dismiss(animated: true) {
                if let iec = self.storyboard?.instantiateViewController(withIdentifier: "iec") as? ImageEditorController{
                    iec.loadViewIfNeeded()
                    iec.imageView.image = imageCaptured
                    iec.delegate = self
                    DispatchQueue.main.async {
                        self.present(iec, animated: true, completion: {
